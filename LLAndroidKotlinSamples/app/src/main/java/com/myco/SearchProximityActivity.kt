...
class SearchProximityActivity  : AppCompatActivity() {
    ...
    private fun initLocusMaps() {
        ...
        LLDependencyInjector.singleton.onPOIURLClickedListener = object : LLOnPOIURLClickedListener {
            override fun onPOIURLClicked(url: String) {
                ...
                this@SearchProximityActivity.startActivity(intent)
            }
        }
    ...
    private fun mapReady() {
        ...
        poiDatabase.getSearchResults("lax", listOf<List<String>>(searchTerms), 33.94221, -118.40205, "lax-south-departures", Locale.getDefault(), object : LLOnGetSearchResultsCallback {
            override fun successCallback(list: List<LLPOI>) {
                var message = ""
                for (poi in list) {
                    message = """$message${poi.name} (${poi.description})
"""
                }
                val dialog = AlertDialog.Builder(this@SearchProximityActivity)
